...
void loop() {
  ...
  if (millis() - colorReadStart >= colorReadInterval || colorReadStart == 0) {
    ...
    if (millis() - colorReadStart >= colorReadInterval) {
      ...
      if (g > 40) {
        // Turn servo counter clockwise by 90 degrees
        setServoPosition(45); // Set servo position to 0 degrees
        Serial.println("Servo works bro");
      } else {
        // Turn servo clockwise by 90 degrees
       setServoPosition(180); // ADDED - Adjust the value according to your servo's calibration
       Serial.println("Nah bro");
      }
